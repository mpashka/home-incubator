...
extension ReceiptToJson on Receipt {
  Map<String, dynamic> toJson() => {
    ...
    'posTime': posTime.toIso8601String(),
    'total': total,
    'imagePath': imagePath,
...
extension ReceiptItemToJson on ReceiptItem {
  Map<String, dynamic> toJson() => {
    'id': id,
    'receiptId': receiptId,
    'name': name,
...
class SyncService {
  ...
  Future<void> sendAllReceiptsWithItems() async {
    ...
    for (final receipt in receipts) {
      final items = await (db.select(db.receiptItems)..where((tbl) => tbl.receiptId.equals(receipt.id))).get();
