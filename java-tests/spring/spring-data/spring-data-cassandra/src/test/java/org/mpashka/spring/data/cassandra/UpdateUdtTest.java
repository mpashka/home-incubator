...
public class UpdateUdtTest {
    ...
    public void testUpdateIn() {
        ...
        cql.getCqlOperations().execute((SessionCallback<Object>) s -> {
            PreparedStatement prepared = s.prepare("UPDATE attachment SET data.status=:status WHERE message_id=:messageId and attachment_id in :attachmentIds");
            BoundStatement bound = prepared.bind()
                    .setString("status", "Hello3")
                    .setLong("messageId", 5)
                    .setList("attachmentIds", List.of(6L, 7L), Long.class);
            ResultSet result = s.execute(bound);
            result.forEach(r -> log.info("Result: {}", r));
            return result;
        });
