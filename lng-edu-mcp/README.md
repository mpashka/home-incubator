...
[`infra/.env.example`](infra/.env.example) и `backend/src/main/resources/application.yml`):
...
```bash
export JAVA_HOME=/opt/java/jdk-25
./gradlew :backend:test    # unit- и slice-тесты (JUnit 5 + Mockito, без Docker)
./gradlew :backend:check   # полная проверка
```
...
(reported as skipped), когда Docker недоступен, поэтому `:backend:check` остаётся зелёным
и без Docker.
